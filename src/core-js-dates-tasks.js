...
function getNextFriday(date) {
  ...
  const week = 7;
  const add = (5 - currentDay + week) % week || week;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const parseSt = Date.parse(dateStart);
  const parseEnd = Date.parse(dateEnd);
  return Math.floor((parseEnd - parseSt) / (1000 * 60 * 60 * 24)) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const pst = Date.parse(start);
  const pend = Date.parse(end);
  const pdate = Date.parse(date);

  if (pdate >= pst && pdate <= pend) return true;
...
function formatDate(date) {
  ...
  let hours = ddate.getUTCHours();
  let min = ddate.getUTCMinutes();
  let sec = ddate.getUTCSeconds();
  if (min < 10) min = `0${min}`;
  if (sec < 10) sec = `0${sec}`;
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = firstDay; day <= lastDay; day.setDate(day.getDate() + 1)) {
    const dayCheck = day.getDay();

    if (dayCheck === 6 || dayCheck === 0) {
      countDay += 1;
    }
  }
...
function getWeekNumberByDate(date) {
  ...
  const days = (date - firstDayOfYear) / msecOfDay;
  const weekNumber = Math.ceil(days / 7);

  if (firstDayOfYear.getDay() === date.getDay()) {
    return weekNumber + 1;
  }
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  let newDate = new Date(year, month, 13).getDay();

  while (newDate !== 5) {
    month += 1;
    newDate = new Date(year, month, 13).getDay();
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const workSchedule = schedule.filter(
    (d, index) => index % (countWorkDays + countOffDays) < countWorkDays
  );
